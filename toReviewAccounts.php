...
if(isset($_GET['deactivateID'])){
    ...
    $_GET=array();
    $_POST['del_msg'] = "<p>Delete Account of <span class='text-danger'>{$readFName['fname']}</span>? </p><a class='btn btn-danger' href='toReviewAccounts.php?$allGet'>Yes, delete account</a>";
    ...
    <div class="min-h-500 pt-5">
        ...
        <?php }?>
            ...
            <div class="col-12 px-0">
                <h4 class="p-2 bg-lightgreen">Accounts to Review</a></h4>
                <div class="row p-2 m-2">
                    <div class="col-md-6 col-12 col-xl-4 offset-md-3 offset-lg-4">
                        <div class="input-group">
                            <input type="text" class="form-control" placeholder="Search for username, lastname or firstname..." id="searchcAcc2">
                        ...
                        <div class="position-absolute w-100" id="searchResults2">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table table-hover table-bordered table-striped text-center">
                        ...
                        <?php
                            $countAll = $acModel->countNotVerifiedAccounts();
                                    $currentPage=(isset($_GET['page']))? intval($_GET['page']) : 1;
                            $limit = 10;
                            $offset = ($currentPage>1)? ($currentPage-1) * $limit : 0;
                            if($countAll>0){
                                $fetchAccounts = $acModel->readNotVerifiedAccounts($offset, $limit);
                                foreach($fetchAccounts as $GETinfo){
                                    ...
                                    echo '<td><div class="btn-group btn-group-sm" role="group"><a class="btn btn-info" href="toReviewAccounts.php?readID='.$GETinfo['id'].'&page='.$currentPage.'">Read</a>';
                                    echo '<a class="btn btn-warning" href="toReviewAccounts.php?activateID='.$GETinfo['id'].'&page='.$currentPage.'">Activate</a>';
                                    echo '<a class="btn btn-danger" href="toReviewAccounts.php?deleteID='.$GETinfo['id'].'&page='.$currentPage.'">Delete</a>';
                                    echo '</div></td>';
                                    echo '</tr>';
                                }
                            } else 
                                echo '<tr><td colspan="4" class="bg-warning">Nothing to review... </td></tr>';
                            $page_url='toReviewAccounts.php?';
                            $GETpagename='page';
                        ?>                       
                    </table>  
                </div>
